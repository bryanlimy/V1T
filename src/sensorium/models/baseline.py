from sensorium.models.neuralpredictors import utils, encoder, shifters
from sensorium.models.neuralpredictors.core import Stacked2dCore
from sensorium.models.neuralpredictors.readouts import multi_readouts
...
def stacked_core_full_gauss_readout(
    ...
    session_shape_dict = utils.get_dims_for_loader_dict(dataloaders)
    ...
    grid_mean_predictor, grid_mean_predictor_type, source_grids = utils.prepare_grid(
    ...
    in_shapes_dict = {
        k: utils.get_module_output(core, v[in_name])[1:]
        for k, v in session_shape_dict.items()
    }

    readout = multi_readouts.MultipleFullGaussian2d(
    ...
    if shifter is True:
        ...
        if shifter_type == "MLP":
            ...
            shifter = shifters.StaticAffine2dShifter(
                data_keys=data_keys,
                input_channels=input_channels_shifter,
                bias=shifter_bias,
                gamma_shifter=gamma_shifter,
    ...
    model = encoder.FiringRateEncoder(
        core=core,
        readout=readout,
        shifter=shifter,
